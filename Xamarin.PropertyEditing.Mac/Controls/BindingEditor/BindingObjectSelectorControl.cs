...
namespace Xamarin.PropertyEditing.Mac
{
	internal class BindingObjectSelectorControl : NSView
	{
		...
		{
			private ICollection<object> viewModel;
		...
		{
			public ICollection<object> ViewModel { get; }

			internal ObjectOutlineViewDataSource (ICollection<object> viewModel)
			{
				if (viewModel == null)
			...
			viewModel.PropertyChanged += (sender, e) => {
				if (e.PropertyName == nameof (CreateBindingViewModel.ShowObjectSelector)) {
					...
					if (viewModel.ShowObjectSelector && viewModel.SelectedObjects != null) {
						this.objectOutlineView.ViewModel = viewModel.SelectedObjects;
					};
				}
			};
		}
